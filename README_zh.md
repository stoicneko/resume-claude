...
基于 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) + [Typst](https://typst.app/) + [brilliant-cv](https://github.com/mintyfrankie/brilliant-cv) 的 AI 驱动简历管理系统。
...
experiences/*.md        (Markdown — 唯一数据源)
       ↓  /generate-resume [职位描述]
modules_en/*.typ        (AI 生成的 Typst 模块)
modules_zh/*.typ
       ↓  typst compile
output/*.pdf            (可直接投递的简历)
```
...
3. Typst 使用 brilliant-cv 模板将模块编译为精美的 PDF。
...
- 字体：Source Sans 3、Roboto（西文）；Noto Sans CJK SC（中文）

### 构建

```bash
make en          # 构建英文 PDF  → output/resume-en.pdf
make zh          # 构建中文 PDF  → output/resume-zh.pdf
make all         # 构建全部（先清理）
make watch-en    # 监听模式 — 文件变更时自动重新构建
make watch-zh
```
...
| `/generate-resume [职位]` | 根据目标职位描述生成定制化 Typst 模块 |
...
├── modules_en/           # AI 生成的 Typst 模块（英文）
├── modules_zh/           # AI 生成的 Typst 模块（中文）
├── jobs/                 # 目标职位描述
├── assets/logos/         # 公司 Logo（可选）
├── cv.typ                # Typst 主入口
├── metadata.toml         # brilliant-cv 配置
├── Makefile              # 构建命令
...
编辑 `cv.typ` 中的 `import-modules(...)` 调用：
...
| [brilliant-cv](https://github.com/mintyfrankie/brilliant-cv) | 3.2.0 | 简历模板（首次编译自动下载） |
